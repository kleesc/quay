...
func NewWorker(
	...
	return &Worker{
		...
		conn:        conn,
		metrics:     m,
		logger:      logger.With(zap.String("worker", "gc")),
	...
	for {
		...
		case <-ticker.C:
			if err := w.performGarbageCollection(ctx); err != nil {
				...
				w.metrics.RecordWorkerError("gc", "collection", "collection_error")
			}
			...
			if w.config.Features.ImageExpiryTrigger {
				if err := w.scanImageExpiryNotifications(ctx); err != nil {
					...
					w.metrics.RecordWorkerError("gc", "collection", "notification_error")
				}
			}
		}
	}
...
func (w *Worker) Stop(ctx context.Context) error {
	w.logger.Info("Stopping garbage collection worker")
	w.cancel()
	return nil
}

// performGarbageCollection performs garbage collection on repositories
func (w *Worker) performGarbageCollection(ctx context.Context) error {
...
func (w *Worker) getRandomGCPolicy(ctx context.Context) (*int, error) {
	query := `
...
func (w *Worker) findRepositoryWithGarbage(ctx context.Context, gcPolicyS int) (*database.Repository, error) {
...
func (w *Worker) garbageCollectRepository(ctx context.Context, repo *database.Repository) (bool, error) {
	hadChanges := false
...
func (w *Worker) lookupUnrecoverableTags(ctx context.Context, repositoryID int) ([]database.Tag, error) {
	query := `
...
func (w *Worker) lookupExpiredUploadedBlobs(ctx context.Context, repositoryID int) ([]database.UploadedBlob, error) {
	query := `
...
func (w *Worker) purgeExpiredTags(ctx context.Context, tags []database.Tag) error {
	if len(tags) == 0 {
		return nil
	}
...
func (w *Worker) purgeExpiredUploadedBlobs(ctx context.Context, blobs []database.UploadedBlob) error {
	if len(blobs) == 0 {
		return nil
	}
...
func (w *Worker) deleteTag(ctx context.Context, tx *sql.Tx, tagID int) error {
	// Delete the tag
...
func (w *Worker) scanImageExpiryNotifications(ctx context.Context) error {
	...
	for {
		...
		case <-ticker.C:
			...
			if err := w.conn.HealthCheck(w.ctx); err != nil {
				...
				w.metrics.RecordWorkerError("gc", "collection", "health_check_failed")
			}
		}
	}
}
