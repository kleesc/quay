...
// NamespaceGCWorker handles namespace garbage collection
type NamespaceGCWorker struct {
	*WorkerBase
}

// NewNamespaceGCWorker creates a new namespace GC worker
func NewNamespaceGCWorker(
	cfg *config.Config,
	...
	base := NewWorkerBase("namespace-gc", cfg.Queue.NamespaceGC.Name, cfg, conn, q, m, logger)
	
	return &NamespaceGCWorker{
		WorkerBase: base,
	}
}

// Start begins the worker's main processing loop
func (w *NamespaceGCWorker) Start(ctx context.Context) error {
	return w.Run(w.processQueueItem, 
		w.config.Queue.NamespaceGC.PollPeriod,
		w.config.Queue.NamespaceGC.Timeout,
		w.config.Queue.NamespaceGC.MaxRetries)
}

// processQueueItem processes a single namespace GC job
func (w *NamespaceGCWorker) processQueueItem(ctx context.Context, queueItem *queue.QueueItem) error {
	// Parse job details
...
func (w *NamespaceGCWorker) deleteNamespaceViaMarker(ctx context.Context, markerID string) (bool, error) {
...
func (w *NamespaceGCWorker) deleteNamespaceRepositories(ctx context.Context, tx *sql.Tx, namespaceID int) error {
...
func (w *NamespaceGCWorker) deleteRepository(ctx context.Context, tx *sql.Tx, repositoryID int) error {
...
func (w *NamespaceGCWorker) deleteNamespaceData(ctx context.Context, tx *sql.Tx, namespaceID int) error {
	// Delete team members
...
func (w *NamespaceGCWorker) deleteUser(ctx context.Context, tx *sql.Tx, userID int) error {
...
func (w *NamespaceGCWorker) deleteMarker(ctx context.Context, tx *sql.Tx, markerID int) error {
