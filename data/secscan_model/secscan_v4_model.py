...
class V4SecurityScanner(SecurityScannerInterface):
    ...
    def _index(self, iterator, reindex_threshold):
        ...
        for candidate, abt, num_remaining, should_skip in batched_iterator_with_preemption_check(iterator):
            ...
            if should_skip:
                logger.debug(
                    "Manifest %d preempted by another worker (batch check)", candidate.id
                )
                abt.set()
                continue
