...
def test_batch_preemption_check(initialized_db, set_secscan_config):
    ...
    assert indexed_count < len(manifests), f"Expected some manifests to be skipped, but indexed {indexed_count} out of {len(manifests)}"
...
def test_batched_iterator_with_preemption_check(initialized_db, set_secscan_config):
    ...
    assert indexed_count < len(manifests), f"Expected some skipped, but indexed {indexed_count}/{len(manifests)}"
...
def test_batch_preemption_reduces_queries(initialized_db, set_secscan_config):
    ...
    # With batch_size=20, we expect approximately:
    # 50 manifests / 20 per batch = 3 batches = 3 queries
    # Without batching, it would be 50 queries
    # This is a 16-17x improvement!
